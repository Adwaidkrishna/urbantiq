window.AuthGuard = (() => {
    ...
    function initSearchSuggestions() {
        ...
        searchContainers.forEach(container => {
            ...
            let suggestionsDiv = container.querySelector('.search-suggestions');
            if (!suggestionsDiv) {
                ...
                try {
                    ...
                    suggestionsDiv.innerHTML = data.products.map(p => {
                        ...
                        if (offerPrice && offerPrice < price) {
                            priceHtml = `<span class="suggestion-price suggestion-price-discounted">₹${offerPrice.toLocaleString('en-IN')}</span>
                                         <span class="suggestion-price-original" style="text-decoration: line-through; font-size: 0.75rem; color: #86868b; margin-left: 6px;">₹${price.toLocaleString('en-IN')}</span>`;
                        } else {
                            priceHtml = `<span class="suggestion-price">₹${price.toLocaleString('en-IN')}</span>`;
                        }
